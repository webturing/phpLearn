...
<?php
class Tools {
	public static function prime($n) {
		for($c = 2; $c <= sqrt ( $n ); $c ++)
			if ($n % $c == 0)
				return FALSE;
		return TRUE;
	}
	public static function int2bool($i) {
		return $i == 0;
	}
	public static function bool2in($b) {
		return $b ? 1 : 0;
	}
	public static function imply($p, $q) {
		return ! $p || $q;
	}
	public static function true($arr) {
		foreach ( $arr as $item )
			if (! $item)
				return false;
		return true;
	}
	public static function solve($a, $b, $c, $n, $m) {
		for($x = 0; $x <= $n; $x ++)
			for($y = 0; $y <= $n; $y ++) {
				$z = $n - $x - $y;
				if ($x * $a + $y * $b + $z * $c == $m)
...
?>
<BODY>
	<h1>程序设计基础题P20 in PHP</h1>
	<H2>1. 随机产生一些1-100之间的整数，直到产生的数为50为止。</H2>
 <?php
	class P01 {
		public $start, $end, $target;
		public function __construct($start, $end, $target) {
			$this->start = $start;
			$this->end = $end;
			$this->target = $target;
		}
		function main() {
			while ( ($current = rand ( $this->start, $this->end )) != $this->target )
				echo $current . "->";
			echo $this->target . "<br>";
		}
	}
	
	$p01 = new P01 ( 1, 100, 50 );
	$p01->main ();
	?>
 <H2>2. 计算1-1000之间能同时被3和5整除的整数的和。</H2>
<?php
class P02 {
	public $start, $end, $p, $q;
	public function __construct($start, $end, $p, $q) {
		$this->start = $start;
		$this->end = $end;
		$this->p = $p;
		$this->q = $q;
	...
	function main() {
		for($s = 0, $i = $this->start; $i <= $this->end; $i ++)
			if ($i % $this->p == 0 && $i % $this->q == 0)
				$s += $i;
		echo $s . "<br>";
	}
}

$p02 = new P02 ( 1, 1000, 3, 5 );
$p02->main ();
?>
...
$p03->main ()?>
 ...
 <?php
	class P06 {
		static function left($p, $q) {
			return $p + 1 == $q || $p == 5 && $q == 1;
		}
		static function right($p, $q) {
			return self::left ( $q, $p );
		}
		static function adj($p, $q) {
			return self::left ( $p, $q ) || self::right ( $p, $q );
		}
				...
				for($sun = 2; $sun <= $n; $sun ++)
					for($li = 2; $li <= $n; $li ++) {
						...
						if ($qian == $sun || $qian == $li || $qian == $zhou || $sun == $li || $sun == $zhou || $li == $zhou)
							continue;
						$ap = self::adj ( $zhao, $qian );
						$aq = self::left ( $sun, $qian ) || self::left ( $li, $qian );
						$bp = self::left ( $qian, $sun );
						$bq = self::adj ( $li, $sun );
						if (! $ap && ! $aq && ! $bp && ! $bq) {
 ...
 <h2>7.
	...
	function p07() {
		for($n = 100; $n <= 999; $n ++) {
			...
			$b = ($n % 100 - $c) / 10;
			$a = ($n - $b * 10 - $c) / 100;
			if ($a != $b && $b != $c && $c != $a && $b > ($a + $c) && ! Tools::prime ( $a + $b ))
				echo ($n . "&nbsp");
		}
		echo ("<br>");
	}
					...
					for($d = 0; $d <= 1; $d ++)
						for($e = 0; $e <= 1; $e ++) {
							$t = array (
									$a + $c <= 1,
									$b + $c > 0,
									Tools::imply ( $c == 1, $d + $e == 1 ),
									$b + $c + $d <= 2,
									Tools::imply ( $b == 1, $d + $e <= 1 ),
									$a + $b + $c + $d + $e == 3 
							);
...
?>
	...
	<h2>12. 截数问题:
		任意一个自然数，我们可以将其平均截取成三个自然数。例如自然数135768，可以截取成13,57,68三个自然数。如果某自然数不能平均截取(位数不能被3整除)，可将该自然数高位补零后截取。现编程从键盘上输入一个自然数N(N的位数
		<12)，计算截取后第一个数加第三个数减第二个数的结果。</h2>
	<?php
	$n = '1357681';
	$len = strlen ( $n );
	for($i = 0; $i < ($len % 3); $i ++) {
...
p18 ( 202 );
?>
	<h2>19. 从键盘上输入两个不超过32767的整数，试编程序用竖式加法形式显示计算结果。 例如: 输入 123, 85 显示: 123 +
		85 ------------- 208</h2>
	<?php
	function p19($ia, $ib) {
